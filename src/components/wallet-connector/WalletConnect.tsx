...
const WalletConnect = ({ Width }: WalletConnectDetail) => {
  ...
  const getNFTList = async () => {
    const config: NftStoreConfig = {
      paymentMint: new PublicKey(
        process.env.NEXT_PUBLIC_PAYMENT_MINT as string
      ),
  ...
  if (!wallet || content == null)
    return (
      <button
        ...
        className={`-bg-gradient-to-r inline-flex items-center gap-x-2 rounded-md bg-purple-2 from-purple-1 to-purple-2 px-4 py-2 text-base font-medium text-white hover:bg-white hover:from-purple-2 hover:to-purple-1 hover:text-purple-2 ${
  ...
  return (
    ...
    >
      ...
      >
        <Menu.Items className="absolute right-0 mt-2 w-60 origin-top-right divide-y divide-gray-100 rounded-lg rounded-md bg-[#231f38] shadow-lg shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
          <div className="relative grid gap-6 gap-8 px-7 py-8">
            ...
            <Menu.Item>
              <a
                ...
                className="-m-3 flex cursor-pointer items-center rounded-lg p-3 hover:bg-white hover:bg-opacity-5"
              >
                <span
                  className="h-6 w-6 flex-shrink-0 text-purple-2"
                  aria-hidden="true"
