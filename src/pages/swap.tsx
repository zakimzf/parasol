...
export default () => {
  ...
  const getRoutes = async (am = 0) => {
    ...
    console.log(computeRoutes.routesInfos);
    setRoutes(computeRoutes.routesInfos);
  };

  const startSwap = async () => {
    console.log(inputAmount);
    console.log("input===", chosenInput);
    console.log("output===", chosenOutput);
    console.log(routes);
  };
  const getMaxAmount = () => {
    if (wallet.connected) {
    ...
    if (wallet.connected) {
      return (
        ...
        >
          ...
          Swap {chosenInput && chosenInput.symbol} to{" "}
    ...
    <section className="pt-6 pb-20">
      <div className="flex flex-col gap-y-6 max-w-md mx-auto mt-6">
        <div className="bg-[#231f38] bg-opacity-80 shadow-xl rounded-xl shadow-half-strong border border-gray-800 p-8">
          ...
          <svg
            className={"mx-auto h-4 mt-5 mb-2 text-gray-300"}
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 20 20"
          >
            <path
